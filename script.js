  ...
  if (button.innerText == 'Restart') {
    ...
    if (input.value != '') {
      if (input.value == number) {
        ...
        info.innerText = 'The number you entered is less than my number try entering some bigger values';
        Tries_left -= 1;
        try_info.innerText = tryf(Tries_left);
      }
      else if (input.value > number) {
        info.innerText = 'The number you entered is bigger than my number try entering some less values';
        Tries_left -= 1;
        try_info.innerText = tryf(Tries_left);
      }
    }
